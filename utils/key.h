...
namespace ecdsa {
    ...
    Key::Key() {
        ...
        CalculatePublicKey(Key::isPubCompressed);
    }
    ...
    bool Key::CalculatePublicKey(bool compressed) {
        ...
        secp256k1_ec_pubkey_serialize(ctx_, pub_key_data_.data(), &out_size, &pubkey, compressed ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED);
        pub_key_data_.resize(out_size);
        return true;
    }
